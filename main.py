...
class Vector:
    ...
    def crossProd(self, right):
        return Vector(
            self.y * right.z - right.y * self.z,
            -(self.x * right.z - right.x * self.z),
...
class Point(Vector):
    ...
    def dist(self, right) -> float:
        ...
        """
        return math.sqrt(
            (right.x - self.x) ** 2 + (right.y - self.y) ** 2 + (right.z - self.z) ** 2
        )
...
class Force:
    def __init__(self, strength: Union[Vector, List[float]]) -> None:
        ...
        if type(strength) == list:
            if len(strength) > 3:
                print(
                    "the length of the array is longer than 3. \n Deprecating the",
                    "array to size 3",
                )
            x = strength[0]
            y = strength[1]
...
class Body:
    def __init__(
        self, mass: float, pos: Point, vel: Vector, acc: Vector, t: Union[float, int] = 0
    ) -> None:
        self.mass = mass
        self.position = pos
        self.vel = vel
